...
start_idx = content.find('@app.on_event("startup")')
shutdown_idx = content.find('@app.on_event("shutdown")')

if start_idx != -1 and shutdown_idx != -1:
    new_content = content[:start_idx] + new_loops + "\\n" + startup_event_new + "\\n\\n" + content[shutdown_idx:]
    with open('src/flose/web_app.py', 'w') as f:
        f.write(new_content)
    print("Injected new loops and startup event successfully!")
else:
    print("Could not find startup or shutdown event markers.")
